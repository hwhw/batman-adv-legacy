...
#ifndef _NET_BATMAN_ADV14_UNICAST_H_
#define _NET_BATMAN_ADV14_UNICAST_H_
...
/**
 * batadv14_unicast_send_skb - send the skb encapsulated in a unicast packet
 * @bat_priv: the bat priv with all the soft interface information
 * @skb: the payload to send
 */
static inline int batadv14_unicast_send_skb(struct batadv14_priv *bat_priv,
					  struct sk_buff *skb)
{
	return batadv14_unicast_generic_send_skb(bat_priv, skb, BATADV_UNICAST,
					       0);
}

/**
 * batadv14_unicast_send_skb - send the skb encapsulated in a unicast4addr packet
 * @bat_priv: the bat priv with all the soft interface information
 * @skb: the payload to send
 * @packet_subtype: the batman 4addr packet subtype to use
 */
static inline int batadv14_unicast_4addr_send_skb(struct batadv14_priv *bat_priv,
						struct sk_buff *skb,
						int packet_subtype)
{
	return batadv14_unicast_generic_send_skb(bat_priv, skb,
					       BATADV_UNICAST_4ADDR,
					       packet_subtype);
}

static inline int batadv14_frag_can_reassemble(const struct sk_buff *skb, int mtu)
{
	const struct batadv14_unicast_frag_packet *unicast_packet;
	...
	merged_size += sizeof(struct batadv14_unicast_packet) + uneven_correction;

	return merged_size <= mtu;
}

#endif /* _NET_BATMAN_ADV14_UNICAST_H_ */
